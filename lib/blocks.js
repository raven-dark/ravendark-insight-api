...
BlockController.prototype.getBlockReward = function(prevHash, cb) {
  var self = this;
  var nInitialReward = 200;
  var nDecrementInterval = 525600;
  var nDecrementPct = 0.00488;
  ...
  self.getPreviousBlock(prevHash, function(err, info) {
    ...
    if(info===null){
        cb(null, parseFloat(nInitialReward.toString(10)).toFixed(8));
    }
    else{
	    var nPrevHeight = info.height;
      var decrementPct = Math.floor(nPrevHeight / nDecrementInterval) * nDecrementPct;
      var reward = nInitialReward - (nInitialReward * decrementPct);
